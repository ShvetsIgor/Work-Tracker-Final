...
const Header = ({ onOpenAccount, onOpenSettings }) => {
  ...
  return (
    <header
      className={`sticky top-0 z-[70] border-b px-0 pt-[var(--header-safe-top)] lg:border-b-0 lg:px-8 lg:pt-6 ${
        isDark ? 'theme-bg-header border-white/[0.05]' : 'theme-bg-header border-slate-200/80'
      }`}
      style={{ '--header-safe-top': mobileSafeTop }}
    >
      <div className="mx-auto flex items-center justify-between lg:max-w-7xl lg:pl-[18rem]">
        <div className={`flex w-full items-center justify-between gap-3 px-3.5 py-2.5 transition-colors duration-300 lg:rounded-[16px] lg:border ${
          isDark
